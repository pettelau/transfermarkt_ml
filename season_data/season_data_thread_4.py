...
def season_data(pid, player_code):
    ...
    renamed["Minutes_per_goal"] = (
        renamed["Minutes_per_goal"].astype(str).str.replace(r"\D", "").astype(int)
    )

    ## Minutes played
    renamed["Minutes_played"] = renamed["Minutes_played"].astype(str).str.replace("-", "0")

    renamed["Minutes_played"] = (
        renamed["Minutes_played"].astype(str).str.replace(r"\D", "").astype(int)
    )
...
for index, row in df2.iterrows():
    ...
    try:
        ...
        appended_data.to_csv("season_data_17_18.csv", index=False)
        print(e)
